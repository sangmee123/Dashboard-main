    ...
    */
    ...
    if($conn){ 
        ...
        if ($result = mysqli_query($conn, $sql)) {
            ...
            $msg = json_encode(array("result" => 'success',"Error" => 'None',"idx" => $row["idx"],
            "module_idx" => $row["module_idx"],"pressure" => $row["pressure"],"torque" => $row["torque"],
            "inclination_x" => $row["inclination_x"],"inclination_y" => $row["inclination_y"],"max_depth" => $row["max_depth"],
            "collecttime" => $row["collecttime"],"beat_position" => $row["beat_position"]));
        } else {
